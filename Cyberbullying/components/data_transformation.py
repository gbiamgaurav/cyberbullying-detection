...
class DataTransformation:
    ...
    def labeled_data_cleaning(self):
        try:
            logging.info("Entered into the raw_data_cleaning function")
            labeled_data = pd.read_csv(self.data_ingestion_artifacts.labeled_data_file_path)
            labeled_data.drop(self.data_transformation_config.DROP_COLUMNS,axis = self.data_transformation_config.AXIS,
            inplace = self.data_transformation_config.INPLACE)

            labeled_data[labeled_data[self.data_transformation_config.CLASS]==0][self.data_transformation_config.CLASS]=1
            
            # replace the value of 0 to 1
            labeled_data[self.data_transformation_config.CLASS].replace({0:1},inplace=True)

            # Let's replace the value of 2 to 0.
            labeled_data[self.data_transformation_config.CLASS].replace({2:0}, inplace = True)

            # Let's change the name of the 'class' to label
            labeled_data.rename(columns={self.data_transformation_config.CLASS:self.data_transformation_config.LABEL},inplace =True)
            logging.info(f"Exited the raw_data_cleaning function and returned the raw_data {labeled_data}")
            return labeled_data
    ...
    def concat_dataframe(self):
        try:
            ...
            frame = [self.labeled_data_cleaning(), self.imbalanced_data_cleaning()]
            df = pd.concat(frame)
            print(df.head())
